--修改保险箱密码界面
local ControllerSafeBoxChangePasswd = class("ControllerSafeBoxChangePasswd")

function ControllerSafeBoxChangePasswd:Init()
	self.m_view=UIPackage.CreateObject('hall','safeBoxChangePasswdView').asCom
	UIManager.normal:AddChild(self.m_view)
	self.m_view:SetSize(GRoot.inst.width, GRoot.inst.height)
    self.m_view.visible=false
    
    self.m_view:GetChild("btnClose").onClick:Add(function ()
        UIManager.Hide('ControllerSafeBoxChangePasswd')
	end)
...
function ControllerSafeBoxChangePasswd:Show(arg)
	--保持在前面
	UIManager.normal:AddChild(self.m_view)
...
function ControllerSafeBoxChangePasswd:OnHide()
	self.m_view.visible=false
end
return ControllerSafeBoxChangePasswd