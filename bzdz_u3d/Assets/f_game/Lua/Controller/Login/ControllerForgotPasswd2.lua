--修改密码界面
local ControllerForgotPasswd2 = class("ControllerForgotPasswd2")

function ControllerForgotPasswd2:Init()
	self.m_view=UIPackage.CreateObject('main','ForgotPasswdView2').asCom
	UIManager.normal:AddChild(self.m_view)
	self.m_view:SetSize(GRoot.inst.width, GRoot.inst.height)
    self.m_view.visible=false
    
    self.m_view:GetChild("btnClose").onClick:Add(function ()
        UIManager.Hide('ControllerForgotPasswd2')
    end)
...
function ControllerForgotPasswd2:Show(arg)
	--保持在前面
	UIManager.normal:AddChild(self.m_view)
...
function ControllerForgotPasswd2:OnHide()
	self.m_view.visible=false
end
return ControllerForgotPasswd2