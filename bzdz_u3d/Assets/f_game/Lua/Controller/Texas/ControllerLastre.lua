--游戏内查看上局回顾
local ControllerLastre = class("ControllerLastre")

function ControllerLastre:Init()
	...
	self.m_view:GetChild("btnClose").onClick:Add(function ()
        UIManager.Hide('ControllerLastre')
    end)
end

function ControllerLastre:Show(arg)
	--保持在前面
	UIManager.normal:AddChild(self.m_view)
	...
	local _str1=string.format( "%s(%s)",roomCfg.tablename,roomCfg.tableid)
	local _str2=nil
	if roomCfg.frontbet>0 then
		_str2=string.format("盲注：%s/%s 前注：%s",roomCfg.xiaomangbet,roomCfg.xiaomangbet*2,roomCfg.frontbet)
	else
		_str2=string.format("盲注：%s/%s",roomCfg.xiaomangbet,roomCfg.xiaomangbet*2)
	end
	...
	for i,v in ipairs(arg.data2.result) do
		...
		--分数
		_gcom:GetChild("txtValue").text=v.win_gold
	end
end

function ControllerLastre:OnHide()
	self.m_view.visible=false
end
return ControllerLastre